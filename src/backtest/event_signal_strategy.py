...
class EventPredictionStrategy(bt.Strategy):
    params = dict(
        ...
        target_percent=0.95,      # 新增：95% 仓位买入
    ...
    def notify_order(self, order):
        ...
        if order.status == order.Completed:
            ...
            if order.isbuy():
                ...
                entry_context["target_percent"] = float(self.p.target_percent)
        ...
        self.trade_log.append(
            {
                ...
                "target_percent": entry_ctx.get("target_percent"),
    ...
    def next(self):
        ...
        if int(event_signal) == 1 and float(signal_prob) >= decision_threshold:
            ...
            # 关键改动：按 95% 仓位买入
            self.order = self.order_target_percent(target=float(self.p.target_percent))